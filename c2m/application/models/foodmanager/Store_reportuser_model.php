...
$query = $this->db->query('SELECT 
    ...
    (SELECT sum(sd.food_num) FROM food_order as sd WHERE sd.food_id=fl.food_id   AND sd.store_id="'.$_SESSION['store_id'].'" AND sd.user_id="'.$user_id.'" AND sd.food_order_status="1" AND sd.create_date BETWEEN "'.$dayfrom.'" AND "'.$dayto.'") as food_numsaleall,
    

    (SELECT sum(sd.food_price * sd.food_num) FROM food_order as sd WHERE sd.food_id=fl.food_id AND sd.user_id="'.$user_id.'"  AND sd.store_id="'.$_SESSION['store_id'].'" AND sd.food_order_status="1" AND sd.create_date BETWEEN "'.$dayfrom.'" AND "'.$dayto.'") as food_pricesaleall
