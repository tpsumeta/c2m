...
$query = $this->db->query('SELECT * FROM food_table WHERE food_brand_id="'.$_SESSION['food_brand_id'].'" ORDER BY food_table_id ASC');
...
$query = $this->db->query('SELECT * FROM food_order WHERE food_brand_id="'.$_SESSION['food_brand_id'].'" AND food_table_id="'.$data['food_table_id'].'" AND food_order_status="0"  ORDER BY food_order_id ASC');
$encode_data = json_encode($query->result(),JSON_UNESCAPED_UNICODE );
return $encode_data;
...
LEFT JOIN food_category as fc on fc.food_category_id=fl.food_category_id
 WHERE fl.food_brand_id="'.$_SESSION['food_brand_id'].'" AND fl.food_status="0" ORDER BY fl.food_id DESC');
$encode_data = json_encode($query->result(),JSON_UNESCAPED_UNICODE );
return $encode_data;
...
$query = $this->db->query('SELECT * FROM food_category WHERE food_brand_id="'.$_SESSION['food_brand_id'].'" ORDER BY food_category_id ASC');
$encode_data = json_encode($query->result(),JSON_UNESCAPED_UNICODE );
return $encode_data;
...
FROM food_list fl
LEFT JOIN food_category as fc on fc.food_category_id=fl.food_category_id
 WHERE fl.food_brand_id="'.$_SESSION['food_brand_id'].'" AND fl.food_status="0" AND fl.food_category_id="'.$data['food_category_id'].'" ORDER BY fl.food_id DESC');
$encode_data = json_encode($query->result(),JSON_UNESCAPED_UNICODE );
return $encode_data;
...
$data['food_brand_id'] = $_SESSION['food_brand_id'];
...
$query = $this->db->query('DELETE FROM food_order  
    ...
    AND  food_brand_id="'.$_SESSION['food_brand_id'].'"');
return true;
