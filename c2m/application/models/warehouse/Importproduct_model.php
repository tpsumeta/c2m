...
$querynum = $this->db->query('SELECT *, from_unixtime(importproduct_header_date,"%d-%m-%Y %H:%i:%s") as importproduct_header_date2
    FROM wh_importproduct_header  
    WHERE owner_id="'.$_SESSION['owner_id'].'"  AND importproduct_header_refcode LIKE "%'.$data['searchtext'].'%" OR importproduct_header_remark LIKE "%'.$data['searchtext'].'%"
    ORDER BY importproduct_header_id DESC LIMIT '.$start.' , '.$perpage.' ');


$query = $this->db->query('SELECT *, from_unixtime(importproduct_header_date,"%d-%m-%Y %H:%i:%s") as importproduct_header_date2
    FROM wh_importproduct_header  
    WHERE owner_id="'.$_SESSION['owner_id'].'"  AND importproduct_header_refcode LIKE "%'.$data['searchtext'].'%" OR importproduct_header_remark LIKE "%'.$data['searchtext'].'%"
    ORDER BY importproduct_header_id DESC LIMIT '.$start.' , '.$perpage.' ');
